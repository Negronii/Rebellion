public class Params {
    public static double initial_cop_density = 0.04;
    public static double initial_agent_density = 0.70;
    public static double government_legitimacy = 0.6;
    public static double vision = 7;
    public static double k = 2.3;
    public static double threshold = 0.10;
    public static int maxJailTerm = 30;
    public static boolean movement = true;

    public static boolean extension = true;
    public static double corruption = -1;
    public static double equipment = 3;
    public static int maxInjuryTerm = 10;
    // length refers to y-axis
    public static int length = 40;
    // width refers to x-axis
    public static int width = 40;
}
